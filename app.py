...
def estimate_bpm(y: np.ndarray, sr: int) -> float:
    try:
        ...
        st.warning(f"No se pudo estimar BPM, usando 120. Error: {e}")
        return 120.0
...
def estimate_key(y: np.ndarray, sr: int) -> Tuple[str, int]:
    try:
        ...
        st.warning(f"No se pudo estimar la tonalidad, usando C Major. Error: {e}")
        return "C Major", 0
...
def apply_time_stretch(y: np.ndarray, rate: float) -> np.ndarray:
    ...
    try:
        ...
        st.warning(f"Fallo time_stretch, usando phase vocoder. Error: {e}")
        hop_length = 512
...
def apply_pitch_shift(y: np.ndarray, sr: int, n_semitones: float) -> np.ndarray:
    ...
    try:
        ...
        st.warning(f"Fallo pitch_shift, usando resample. Error: {e}")
...
def find_beat_position(y: np.ndarray, sr: int, start_time: float) -> float:
    try:
        ...
        st.warning(f"No se pudo encontrar beat exacto. Error: {e}")
        return start_time
